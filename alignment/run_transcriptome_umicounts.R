...
option_list = list(
  ...
  make_option(c("--umifile"), type="character", help="List of UMI sorted like read-sorted bam file with full path", metavar="character"),
...
# opt = list(bamfile="mapping/bwa/SCRBseq/gencode/SCRBseqA.AAAGTT.bam",
# samplename="SCRBseqA.AAAGTT",
# umifile="SCRBseqA.AAAGTT.UMIlist.txt",
...
umis = read.table(file = opt$umifile, stringsAsFactors = F, header=F)
umis = data.frame(id = orig.readid, xm = umis$V1, stringsAsFactors = F)
...
cnts = tbl4counts %>% 
  ...
  dplyr::summarise(TranscriptReads=length(xm), TranscriptUmis = length(unique(xm))) %>%
  dplyr::group_by(gene) %>%
  dplyr::mutate(GeneReads=sum(TranscriptReads), GeneUmis=sum(TranscriptUmis))

counts_full = dplyr::left_join(trans_table, cnts, by="GeneTranscript") %>% 
  dplyr::select(-dplyr::contains("y")) %>%
  tidyr::replace_na(list(TranscriptReads = 0, TranscriptUmis=0, GeneReads=0, GeneUmis=0)) %>%
